...
exports.formatJsonBestEffort = formatJsonBestEffort;
exports.minifyJsonBestEffort = minifyJsonBestEffort;
const EMPTY_RESULT = {
    output: '',
    mode: 'strict'
};
function formatJsonBestEffort(input, indentSize = 2) {
    if (!input.trim()) {
        return EMPTY_RESULT;
    }
    try {
...
function minifyJsonBestEffort(input) {
    if (!input.trim()) {
        return EMPTY_RESULT;
    }
    try {
...
function formatJsonLoosely(input, indentSize) {
    ...
    const findNextNonWhitespace = (startIndex) => {
    ...
    const shouldPreserveSpace = (prev, next) => {
        if (!prev || !next)
            return false;
        if (isStructuralChar(prev) || isStructuralChar(next))
            return false;
        return true;
    };
...
function minifyJsonLoosely(input) {
    const source = input.trim();
    if (!source) {
        return '';
    }
